...
const CURRENT_PRODUCT_GENERIC_GUIDANCE = "Keep the RCA specific to the selected Jira/BugDB ticket, its exact symptom, and the selected Oracle Restaurants product. Do not drift into generic product guidance.";
const CURRENT_EVIDENCE_FIRST_GUIDANCE = "Focus on comments, audit history, attachments, logs, and reproducible evidence before concluding RCA.";
const LEGACY_PRODUCT_GENERIC_GUIDANCE_PATTERN = /Keep the RCA generic to the selected\b[\s\S]*?highlight subsystem-specific evidence\.(?:\s*Do not make any code changes in this RCA run\.\s*If a likely fix is found, report it and wait for an explicit fix request\.)?/g;
const LEGACY_EVIDENCE_FIRST_GUIDANCE_PATTERN = /Focus on comments, audit history, attachments, and logs before concluding RCA\.\s*Do not make any code changes in this RCA run\.\s*If a likely fix is found, report it and wait for an explicit fix request\./g;
...
const TRANSIENT_CODEX_NOISE_PATTERNS = [
  /^thread\.(started|completed|cancelled|failed)\S*/i,
  /^turn\.(started|completed|cancelled|failed)\S*/i,
  /^reading prompt from stdin\.\.\.$/i,
  /^reconnecting\.\.\.\s+\d+\/\d+\s+\(/i,
...
function isTransientCodexNoiseOnly(value) {
  ...
  return lines.every((line) => TRANSIENT_CODEX_NOISE_PATTERNS.some((pattern) => pattern.test(line)));
}
...
function selectPreferredFinalMessage(options = {}) {
  ...
  const rawNormalizedStderr = collapseReportWhitespace(normalizeReportText(stderrText));
  const normalizedStderr = sanitizePersistedOutputText(stderrText)
    || (isTransientCodexNoiseOnly(rawNormalizedStderr) ? "" : rawNormalizedStderr);
...
function sanitizeLegacyPromptText(value) {
  return normalizeReportText(String(value || ""))
    .replace(LEGACY_PRODUCT_GENERIC_GUIDANCE_PATTERN, CURRENT_PRODUCT_GENERIC_GUIDANCE)
    .replace(LEGACY_EVIDENCE_FIRST_GUIDANCE_PATTERN, CURRENT_EVIDENCE_FIRST_GUIDANCE)
    .replaceAll(
...
function resolveEnvPlaceholder(value, env = process.env) {
  ...
  let resolved = normalized.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (match, key) => (
    Object.prototype.hasOwnProperty.call(env, key) ? String(env[key] ?? "") : match
  ));

  resolved = resolved.replace(/^\$([A-Za-z_][A-Za-z0-9_]*)$/g, (match, key) => (
    Object.prototype.hasOwnProperty.call(env, key) ? String(env[key] ?? "") : match
  ));
...
function expandMcpCommandEntry(entry) {
  const command = resolveEnvPlaceholder(String(entry?.command || "").trim());
  const args = Array.isArray(entry?.args)
    ? entry.args.map((value) => resolveEnvPlaceholder(String(value)))
    : [];
  const env = entry?.env && typeof entry.env === "object" && !Array.isArray(entry.env)
    ? Object.fromEntries(
        Object.entries(entry.env)
          .map(([key, value]) => [String(key || "").trim(), resolveEnvPlaceholder(String(value))])
          .filter(([key]) => key)
      )
    : {};
...
function getConfiguredMcpServers() {
  ...
  return Object.entries(servers || {})
    .map(([name, entry]) => {
      ...
      if (isStringEntry) {
        const url = resolveEnvPlaceholder(String(entry || "").trim());
        if (!url) {
          return null;
        }
      ...
      const url = resolveEnvPlaceholder(String(entry?.url || "").trim());
      if (url) {
        return {
          name,
          kind: "url",
      ...
      const { command, args, env } = expandMcpCommandEntry(entry);
