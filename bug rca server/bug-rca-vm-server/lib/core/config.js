...
const CODEX_RUNTIME_HOME_DIR = path.join(RUNTIME_DIR, "codex-home");
...
const RCA_STARTUP_SILENCE_MS = readPositiveMs(process.env.RCA_STARTUP_SILENCE_MS, 60000, 15000);
...
const DEFAULT_CODE_ROOT = process.platform === "win32" ? "C:\\Code" : "";

function localCodeWorkspace(productKey) {
  return DEFAULT_CODE_ROOT && productKey ? path.join(DEFAULT_CODE_ROOT, productKey) : "";
}

function normalizeProductWorkspace(value, productKey) {
  const workspace = String(value || "").trim();
  if (workspace && !workspace.toLowerCase().startsWith("shared://")) {
    return workspace;
  }
  return localCodeWorkspace(productKey);
}

const DEFAULT_SIMPHONY_WORKSPACE = normalizeProductWorkspace(process.env.SIMPHONY_WORKSPACE, "simphony");
const DEFAULT_RNA_WORKSPACE = normalizeProductWorkspace(process.env.RNA_WORKSPACE, "rna");
const DEFAULT_FLM_WORKSPACE = normalizeProductWorkspace(process.env.FLM_WORKSPACE, "flm");
const DEFAULT_CNC_WORKSPACE = normalizeProductWorkspace(process.env.CNC_WORKSPACE, "cnc");
...
const DEFAULT_PRODUCT_OPTIONS = [
  {
    ...
    defaultWorkspace: DEFAULT_SIMPHONY_WORKSPACE,
    usesSkill: true,
  ...
  },
  {
    ...
    defaultWorkspace: DEFAULT_RNA_WORKSPACE,
    usesSkill: false
  },
  {
    key: "flm",
    ...
    defaultWorkspace: DEFAULT_FLM_WORKSPACE,
    usesSkill: false
  },
  {
    key: "cnc",
    label: "C&C",
    family: "Oracle Restaurants C&C",
    defaultWorkspace: DEFAULT_CNC_WORKSPACE,
    usesSkill: false
  }
];
...
function normalizeProductOption(rawProduct, index) {
  ...
  const defaultWorkspace = normalizeProductWorkspace(rawProduct.defaultWorkspace || rawProduct.workspace || "", key);
...
function normalizeSharedWorkspaceOption(rawWorkspace, index) {
  ...
  const rawRootPaths = Array.isArray(rawWorkspace.rootPath)
    ? rawWorkspace.rootPath
    : Array.isArray(rawWorkspace.paths)
      ? rawWorkspace.paths
      : [rawWorkspace.rootPath || rawWorkspace.path || ""];
  const rootPathCandidates = rawRootPaths
    .map((value) => resolveConfiguredWorkspacePath(value))
    .filter(Boolean);
  const rootPath = rootPathCandidates.find((candidate) => fs.existsSync(candidate)) || rootPathCandidates[0] || "";
  if (!rootPath) {
    return null;
  }
...
function expandEnvPathTokens(value) {
  return String(value || "").replace(/%([^%]+)%|\$\{([^}]+)\}|\$([A-Za-z_][A-Za-z0-9_]*)/g, (_match, windowsKey, bracedKey, bareKey) => {
    const envKey = windowsKey || bracedKey || bareKey || "";
    return envKey ? String(process.env[envKey] || "") : "";
  });
}

function resolveConfiguredWorkspacePath(rawValue) {
  const trimmed = String(rawValue || "").trim();
  if (!trimmed) {
    return "";
  }

  const expanded = expandEnvPathTokens(trimmed);
  if (!expanded) {
    return "";
  }
  ...
  return path.isAbsolute(expanded)
    ...
    : path.resolve(APP_ROOT, expanded);
}
...
function loadSharedWorkspaceOptions() {
  ...
  if (rawConfig && !Object.keys(configured).length) {
    console.warn("Ignoring SHARED_WORKSPACE_ROOTS because none of the configured root paths resolved to existing folders.");
  }
